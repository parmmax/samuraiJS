import React from "react";
import classes from './Users.module.css'

const Users = (props) => {

    if (props.users.length === 0) {
        props.setUsers([
            {
                id: 1,
                followed: true,
                avatarUrl: 'data:image/png;base64,',
                fullName: 'Ron G.',
                status: 'Happy Hacking',
                location: {cityName: 'Liverpool', country: 'England'}
            },
            {
                id: 2,
                followed: false,
                avatarUrl: 'data:image/png;base64,',
                fullName: 'Mike F.',
                status: 'Happy Hacking',
                location: {cityName: 'New York', country: 'USA'}
            },
            {
                id: 3,
                followed: false,
                avatarUrl: 'data:image/png;base64,',
                fullName: 'Fred D.',
                status: 'Happy Hacking',
                location: {cityName: 'Melbourne', country: 'Australia'}
            },
            {
                id: 4,
                followed: true,
                avatarUrl: 'data:image/png;base64,',
                fullName: 'Jack B.',
                status: 'Happy Hacking',
                location: {cityName: 'San Francisco', country: 'USA'}
            },
            {
                id: 5,
                followed: false,
                avatarUrl: 'data:image/png;base64,',
                fullName: 'Greg C.',
                status: 'Happy Hacking',
                location: {cityName: 'London', country: 'England'}
            },]
        );
    }

    return (
        <div className={classes.wrapper}>
            <h4>Users</h4>
            {
                props.users.map(u => <div key={u.id}
                                          className={classes.uContainer}>
                    <div className={classes.uAbout}>
                        <div>
                            <img src={u.avatarUrl} alt="User Avatar"/>
                        </div>
                        <div className={classes.uAboutInfo}>
                            <div>{u.fullName}</div>
                            <div>{u.location.cityName}, {u.location.country}</div>
                            <div>{u.status}</div>
                        </div>
                    </div>
                    <div className={classes.uFollow}>
                        {u.followed
                            ? <button className={classes.btnUnFollow}
                                      onClick={() => {
                                          props.unfollow(u.id)
                                      }}>unfollow</button>
                            : <button className={classes.btnFollow}
                                      onClick={() => {
                                          props.follow(u.id)
                                      }}>follow</button>
                        }
                    </div>
                </div>)
            }
        </div>
    )
};

export default Users